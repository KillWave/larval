...
//新建线程
ObjThread* newObjThread(ObjClosure* objClosure) {
   ASSERT(objClosure != NULL, "objClosure is NULL!");

   Frame* frames = ALLOCATE_ARRAY( Frame, INITIAL_FRAME_NUM);

   //加1是为接收者的slot
   uint32_t stackCapacity = ceilToPowerOf2(objClosure->fn->maxStackSlotUsedNum + 1);  
   Value* newStack = ALLOCATE_ARRAY( Value, stackCapacity); 

   ObjThread* objThread = ALLOCATE( ObjThread);
   initObjHeader( &objThread->objHeader, OT_THREAD);
