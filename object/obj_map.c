...
ObjMap *newObjMap()
{
    ObjMap *objMap = ALLOCATE(ObjMap);
    initObjHeader(&objMap->objHeader, OT_MAP);
...
static void resizeMap(ObjMap *objMap, uint32_t newCapacity)
{
    // 1 先建立个新的entry数组
    Entry *newEntries = ALLOCATE_ARRAY(Entry, newCapacity);
    uint32_t idx = 0;
    ...
    // 3 将老entry数组空间回收
    DEALLOCATE_ARRAY(objMap->entries, objMap->count);
